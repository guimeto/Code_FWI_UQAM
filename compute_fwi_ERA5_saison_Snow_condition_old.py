...
for year in range(yi,yf+1):
    ...
    file = '/snow3/dueymes/REANALYSES/ERA5/PR/Daily_Total_PR_17h_QC_'+str(year)
    multi_pr = [f'{file}{month}.nc' for month in ['03','04','05','06','07','08']]
    # variables humidité, temperature et vent à 17h UTC
    file = '/snow3/dueymes/REANALYSES/ERA5/UU_VV_TC/ERA5_QC_TC_WIND_'+str(year)
    multi_air = [f'{file}{month}_17h.nc' for month in ['03','04','05','06','07','08']]
    # humidity
    file = '/snow3/dueymes/REANALYSES/ERA5/HR/ERA5_Humidity_'+str(year)
    multi_hum = [f'{file}{month}_17h.nc' for month in ['03','04','05','06','07','08']]
    # épaisseur de neige quotidienne 
    multi_snow = ['/snow3/dueymes/REANALYSES/ERA5/SWE/Surface/ERA5_'+str(year)+month+'/ERA5_SWE_'+str(year)+month+'_daymean.nc4' for month in ['03','04','05','06','07','08']]
    ...
    ## load mask quebec
    mask = xr.open_mfdataset('mask_QUEBEC.nc')
    data_set=data_set.where(mask.region == 1)
    data_onset=data_onset.where(mask.region == 1)
    del [ prec, hum, wind, temp, lats, lons] 
 
    # ecriture du Netcdf
    data_set.to_netcdf('/snow3/dueymes/REANALYSES/ERA5/FWI_INDICES/FWI/ERA5_FWI_Netcdf/ERA5_FWI_QC_SNOW_New_'+str(year)+'_from_'+str(dtime[0].month)+'_to_'+str(dtime[nt].month)+'.nc')
    data_onset.to_netcdf('/snow3/dueymes/REANALYSES/ERA5/FWI_INDICES/Onset/ERA5_SNOW_Onset_'+str(year)+'_from_'+str(dtime[0].month)+'_to_'+str(dtime[nt].month)+'.nc')
    
    # compute of severity rating SR = 0.0272 * FWI^1.77 
    SR = 0.0272 * np.power(data_set, 1.77)
    SR = SR.rename({'FWI':'SR'})
    
    SR.to_netcdf('/snow3/dueymes/REANALYSES/ERA5/FWI_INDICES/SR/ERA5_SR_QC_SNOW_New_'+str(year)+'_from_'+str(dtime[0].month)+'_to_'+str(dtime[nt].month)+'.nc')
    ...
    SR = SR.sum(dim='time')
    SR.to_netcdf('/snow3/dueymes/REANALYSES/ERA5/FWI_INDICES/SR/ERA5_cumulate_SR_QC_SNOW_New_'+str(year)+'_from_'+str(dtime[0].month)+'_to_'+str(dtime[nt].month)+'.nc')
